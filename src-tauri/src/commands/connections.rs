...
use crate::mongo::{connector, ssh_tunnel};
use std::sync::Mutex;
use tauri::State;
use uuid::Uuid;
...
pub async fn save_profile(
    ...
    if let Some(pos) = profiles.iter().position(|p| p.id == input.profile.id) {
        ...
        ssh_tunnel::validate_profile(&updated).map_err(|e| e.to_string())?;
        profiles[pos] = updated;
    } else {
        let created = input.apply_to(None);
        ssh_tunnel::validate_profile(&created).map_err(|e| e.to_string())?;
